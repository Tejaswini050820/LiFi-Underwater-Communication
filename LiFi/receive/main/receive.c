...
csv_entry_t csv_data[MAX_SENTENCES];
int csv_count = 0;

// Embedded CSV file data
extern const uint8_t csv_start[] asm("_binary_data_csv_start");
extern const uint8_t csv_end[] asm("_binary_data_csv_end");
...
/**
 * @brief Parse CSV file and populate csv_data array.
 */
void parse_csv(void) {
    FILE *stream = fmemopen((void*)csv_start, csv_end - csv_start, "r");
    if (stream == NULL) {
        ESP_LOGE(TAG, "Failed to open CSV data");
        return;
    }

    char line[MAX_SENTENCE_LEN + 20];
    csv_count = 0;
    ...
    while (fgets(line, sizeof(line), stream) && csv_count < MAX_SENTENCES) {
        line[strcspn(line, "\r\n")] = 0;
        
        char *token = strtok(line, ",");
        if (token != NULL) {
            csv_data[csv_count].binary_value = (uint8_t)strtol(token, NULL, 2);
            
            token = strtok(NULL, "");
            if (token != NULL) {
                strncpy(csv_data[csv_count].sentence, token, MAX_SENTENCE_LEN - 1);
                csv_data[csv_count].sentence[MAX_SENTENCE_LEN - 1] = '\0';
                csv_count++;
            }
        }
    }
    ...
    fclose(stream);
    ESP_LOGI(TAG, "Parsed %d entries from CSV", csv_count);
}

/**
 * @brief Find sentence for given binary value.
 */
const char* find_sentence(uint8_t binary_value) {
    for (int i = 0; i < csv_count; i++) {
        if (csv_data[i].binary_value == binary_value) {
            return csv_data[i].sentence;
        }
    }
    return NULL;
}
